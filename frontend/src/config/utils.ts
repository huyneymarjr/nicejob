...
export function getTimeAgo(updatedAt: any) {
    const now = dayjs()
    const updatedTime = dayjs(updatedAt)

    const diffInSeconds = now.diff(updatedTime, "second")
    ...
    const diffInMonths = now.diff(updatedTime, "month")
    if (diffInMonths < 12) {
        return `${diffInMonths} tháng trước`
    }

    const diffInYears = now.diff(updatedTime, "year")
